...
class CBHG(nn.Module):
    ...
    def forward(self, x):
        ...
        print('conv projection1 후 shape :', out.shape)
        out = self.conv1d_proj02(out)
        print('conv projection2 후 shape :', out.shape)
        
        highway_input = out + x
        print('out :{} + x : {} '.format(out.shape, x.shape))
        print('residual connection 후 shape : ', highway_input.shape)
        ...
        print('highway 통과 후 shape : ', out.shape)
        
        out, _ = self.GRU(out)
        print('gru 통과 후 shape : ', out.shape)
...
class Attention_Decoder(nn.Module):
    ...
    def forward(self, enc_vec, dec_vec, atten_GRU_h, gru1_h, gru2_h):
        ...
        print('decoder attention통과 후 shape(c_t + d_t) : ', concat_feature.shape)
        ...
        print('linear 통과한 후 shape : ', out.shape)
        out = out.view(-1, self.reduction_factor, self.num_mel)
        print('mel-spectrogram 5개 생성한 것의 shape : ', out.shape)
...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        print('embedding 후  shape : ', out.shape)
        out = self.pre_net(out)
        print('prenet 통과 후  shape : ', out.shape)
        out = torch.transpose(out, 1, 2)
        print('transpose 시킨 후 shape : ', out.shape)
        out = self.CBHG(out)
...
class Mel_Decoder(nn.Module):
    ...
    def forward(self, enc_vec, decoder_input, is_train = True):
        ...
        print('decoder prenet 통과 후 shape : [bs, Time, hidden_dim]', out.shape)
        bs = out.size()[0]
        ...
        if is_train:
            ...
            print('decoder 반복 개수 -> {} / {} = {} '.format(out.size()[1],self.reduction_factor, iteration_step))
            
            ## Go frame
            description = '''
            Go frame을 생성할 때 가장 처음으로 step1의 out값을 출력하고,
            Attention_Decoder에 들어갈 각 파라미터들을 초기화 해준다.
            이후, decoder 반복 개수만큼 Attention_Decoder를 반복하게 된다.
            '''
            print(description)
            ...
            # for i in range(5):
                ...
                print('---- {}번째 decoder 수행 중 ...  '.format(i))
                ...
                print('그 다음 step decoder input으로 들어가는 shape : ', atn_decoder_input.shape)

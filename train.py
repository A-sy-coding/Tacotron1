...
def generate(model, text, device,  curr, _tt):
...
def main(args):
    ...
    for epoch in range(hp.epochs):
        print('----{} 번째 epoch 학습 : '.format(epoch+1))
        ...
        for i, data in enumerate(dataloader):
            ...
            if current_step % hp.save_step == 0:
                ...
                for _t, text in enumerate(sentences):
                    wav = generate(model, text, device, current_step, _t)
            ...
            if current_step % hp.log_step == 0:
                ...
                wandb.log({'train_mel_loss': mel_loss.item(),

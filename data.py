...
class LJDatasets(Dataset):
    ...
    def __init__(self, csv_file, root_dir):
        ...
        """
        self.landmarks_frame = pd.read_csv(csv_file, sep='|', header=None)[:33]
...
def collate_fn(batch):
    ...
    if isinstance(batch[0], collections.Mapping):
        print('--------------------------- collate_fn ------------------------')
        print('--- collate_fn 함수의 batch 값 : ', len(batch))
        print('--- batch의 첫번째 데이터의 text 값 길이 : ', len(batch[0]['text']))
        print('--- batch의 첫번째 데이터의 wav 값 길이 : ', len(batch[0]['wav']))
        
        keys = list()

        text = [d['text'] for d in batch] # batch_size만큼의 길이를 가지고 있다.
        wav = [d['wav'] for d in batch] # batch_size만큼의 길이를 가지고 있다.

        # PAD sequences with largest length of the batch -> padding 수행
        text = _prepare_data(text).astype(np.int32)
        wav = _prepare_data(wav)
        print('_prepare_data 함수 적용 후 text 길이 : ', len(text[0]))
        print('_prepare_data 함수 적용 후 wav 길이 : ', len(wav[0]))
        
        
        magnitude = np.array([spectrogram(w) for w in wav])
        mel = np.array([melspectrogram(w) for w in wav])
        timesteps = mel.shape[-1]
        print('--- spectrogram shape [batch_size, frequency, time]: ',magnitude.shape)
        print('--- mel spectrogram shape [batch_size, mel_frequency, time]: ',mel.shape)
        ...
        if timesteps % hp.outputs_per_step != 0:
            ...
            print('--- padding spectorgram shape : ', magnitude.shape)
            print('--- padding mel-spectorgram shape : ', mel.shape)
...
def save_wav(wav, path):
  ...
  print('---- 저장할 장소 : {} ------'.format(path))
  sf.write(path, wav.astype(np.int16), hp.sample_rate, format='wav')
